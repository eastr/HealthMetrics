import { useAuth } from '../hooks/useAuth'

export default function LoginScreen() {
  const { configured, loading, error, signIn } = useAuth()

  return (
    <div className="flex min-h-dvh flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm text-center">
        <div className="mb-6 text-5xl">💚</div>
        <h1 className="mb-2 text-2xl font-bold text-primary-800">Health Metrics</h1>
        <p className="mb-8 text-sm text-slate-500">
          Track fatigue, mood, nausea, pain, stiffness, and dizziness throughout your day.
        </p>

        {!configured && (
          <div className="mb-6 rounded-xl bg-amber-50 p-4 text-left text-sm text-amber-800 ring-1 ring-amber-200">
            <p className="font-medium">Setup required</p>
            <p className="mt-1">
              Copy <code className="text-xs">.env.example</code> to{' '}
              <code className="text-xs">.env.local</code> and add your Google OAuth Client
              ID. See README for details.
            </p>
          </div>
        )}

        {error && (
          <div className="mb-4 rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</div>
        )}

        <button
          onClick={() => signIn()}
          disabled={!configured || loading}
          className="flex w-full items-center justify-center gap-3 rounded-xl bg-white py-3 font-semibold text-slate-700 shadow-md ring-1 ring-slate-200 hover:bg-slate-50 disabled:opacity-50"
        >
          <svg className="h-5 w-5" viewBox="0 0 24 24">
            <path
              fill="#4285F4"
              d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
            />
            <path
              fill="#34A853"
              d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
            />
            <path
              fill="#FBBC05"
              d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
            />
            <path
              fill="#EA4335"
              d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
            />
          </svg>
          {loading ? 'Signing in…' : 'Sign in with Google'}
        </button>
      </div>
    </div>
  )
}
