import type { DoseKind, HealthEntry } from '../../types/entry'
import { formatTime } from '../../utils/analytics'
import {
  medicationsByDay,
  medicationFrequency,
  type MedicationDayGroup,
  type MedicationFrequency,
} from '../../utils/analytics'

interface MedicationLogProps {
  entries: HealthEntry[]
  days: number
  kind?: DoseKind
}

function FrequencyCards({
  items,
  days,
  kind,
}: {
  items: MedicationFrequency[]
  days: number
  kind: DoseKind
}) {
  const label = kind === 'vitamin' ? 'vitamins' : 'medications'
  const card =
    kind === 'vitamin'
      ? 'bg-emerald-50 ring-emerald-100 text-emerald-900'
      : 'bg-violet-50 ring-violet-100 text-violet-900'
  const sub = kind === 'vitamin' ? 'text-emerald-700' : 'text-violet-700'

  if (items.length === 0) {
    return <p className="text-sm text-slate-400">No {label} logged in this period.</p>
  }

  return (
    <div className="grid gap-3 sm:grid-cols-2">
      {items.map((item) => (
        <div key={item.name} className={`rounded-lg px-4 py-3 ring-1 ${card}`}>
          <div className="font-semibold">{item.name}</div>
          <div className={`text-sm ${sub}`}>
            {item.count} {item.count === 1 ? 'dose' : 'doses'} in {days} days
          </div>
        </div>
      ))}
    </div>
  )
}

function DailyLog({ groups, kind }: { groups: MedicationDayGroup[]; kind: DoseKind }) {
  const label = kind === 'vitamin' ? 'vitamin' : 'medication'
  const border = kind === 'vitamin' ? 'border-emerald-200' : 'border-violet-200'

  if (groups.length === 0) {
    return <p className="text-sm text-slate-400">No {label} entries in this period.</p>
  }

  return (
    <ul className="space-y-4">
      {groups.map((group) => (
        <li key={group.date}>
          <div className="mb-2 font-medium text-slate-800">{group.label}</div>
          <ul className={`space-y-1.5 border-l-2 pl-3 ${border}`}>
            {group.entries.map((entry) => (
              <li key={entry.id} className="text-sm text-slate-700">
                <span className="tabular-nums text-slate-500">{formatTime(entry.timestamp)}</span>
                {'  '}
                <span className="font-medium">{entry.medication}</span>
                {entry.dose && <span className="text-slate-600"> {entry.dose}</span>}
                {entry.notes && <span className="text-slate-500"> — {entry.notes}</span>}
              </li>
            ))}
          </ul>
        </li>
      ))}
    </ul>
  )
}

export default function MedicationLog({
  entries,
  days,
  kind = 'medication',
}: MedicationLogProps) {
  const byDay = medicationsByDay(entries, days, kind)
  const frequency = medicationFrequency(entries, days, kind)

  return (
    <div className="space-y-6">
      <div>
        <h3 className="mb-3 text-sm font-semibold text-slate-700">Daily log</h3>
        <DailyLog groups={byDay} kind={kind} />
      </div>
      <div>
        <h3 className="mb-3 text-sm font-semibold text-slate-700">Frequency</h3>
        <FrequencyCards items={frequency} days={days} kind={kind} />
      </div>
    </div>
  )
}
